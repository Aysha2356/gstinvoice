...
export default function InvoicePreview() {
  ...
  const draftReminder = async () => {
    ...
    try {
      ...
      setDraftError(err.response?.data?.message || 'Could not generate a reminder draft. Check that OPENAI_API_KEY is set on the server.');
    } finally {
      setDrafting(false);
    }
  };
    ...
    <AppLayout title={`Invoice ${invoice.invoiceNumber}`}>
      <div className="flex-between mb-16" style={{ flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-secondary" onClick={duplicate} disabled={duplicating}>
            {duplicating ? 'Duplicating…' : 'Duplicate'}
          </button>
          {invoice.status !== 'paid' && <button className="btn btn-secondary" onClick={() => setStatus('sent')}>Mark as sent</button>}
          {invoice.status !== 'paid' && <button className="btn btn-secondary" onClick={() => setStatus('paid')}>Mark as paid</button>}
      ...
      <div className="invoice-preview mb-24">
        <div className="invoice-head">
          ...
          <div className="text-right">
            ...
            <div className="meta-label" style={{ marginTop: 6 }}>Due date</div>
        ...
        <div className="form-row mb-24">
          <div>
            ...
            <div className="muted">{[invoice.Client?.city, invoice.Client?.state].filter(Boolean).join(', ')}</div>
          </div>
          <div className="text-right">
        ...
        <table className="list-table mb-16">
          <thead><tr><th>Item</th><th>HSN/SAC</th><th>Qty</th><th>Rate</th><th>SGST</th><th>CGST</th><th>Amount</th></tr></thead>
          <tbody>
        ...
        <div style={{ maxWidth: 320, marginLeft: 'auto' }}>
          <div className="flex-between"><span className="muted">Subtotal</span><span>{formatINR(invoice.subtotal)}</span></div>
          <div className="flex-between"><span className="muted">SGST</span><span>{formatINR(invoice.sgstTotal)}</span></div>
          <div className="flex-between"><span className="muted">CGST</span><span>{formatINR(invoice.cgstTotal)}</span></div>
          <div className="total-band"><span>Total</span><span>{formatINR(invoice.total)}</span></div>
        </div>

        {invoice.notes && (<><hr className="dashed-divider" /><div className="muted">{invoice.notes}</div></>)}
        {invoice.terms && <div className="muted" style={{ marginTop: 8 }}>{invoice.terms}</div>}
      </div>
        ...
        <div className="card">
          <div className="flex-between" style={{ flexWrap: 'wrap', gap: 10 }}>
            <div>
              ...
              <p className="muted" style={{ margin: '4px 0 0' }}>Let AI draft a polite reminder email you can send to {invoice.Client?.name || 'the client'}.</p>
            </div>
            <button className="btn btn-primary" onClick={draftReminder} disabled={drafting}>
              {drafting ? 'Drafting…' : 'Draft reminder with AI'}
            </button>
          </div>
          {draftError && <div className="alert-error" style={{ marginTop: 12 }}>{draftError}</div>}
